...
var express = require('express');
var db = require('../db'); // assuming db is configured as shown above

function fetchTodos(req, res, next) {
  db.query('SELECT * FROM todos WHERE owner_id = ?', [req.user.id], function (err, results) {
    if (err) { return next(err); }
...
router.post('/', function (req, res, next) {
  ...
  db.query('INSERT INTO todos (owner_id, title, completed) VALUES (?, ?, ?)', [
    req.user.id,
    req.body.title,
    req.body.completed == true ? 1 : null
  ], function (err) {
...
router.post('/:id(\\d+)', function (req, res, next) {
  ...
  if (req.body.title !== '') { return next(); }
  db.query('DELETE FROM todos WHERE id = ? AND owner_id = ?', [
    req.params.id,
    req.user.id
  ], function (err) {
    if (err) { return next(err); }
  ...
  db.query('UPDATE todos SET title = ?, completed = ? WHERE id = ? AND owner_id = ?', [
    req.body.title,
    req.body.completed !== undefined ? 1 : null,
    req.params.id,
    req.user.id
...
router.post('/toggle-all', function (req, res, next) {
  db.query('UPDATE todos SET completed = ? WHERE owner_id = ?', [
    req.body.completed !== undefined ? 1 : null,
    req.user.id
  ], function (err) {
    if (err) { return next(err); }
...
router.post('/clear-completed', function (req, res, next) {
  db.query('DELETE FROM todos WHERE owner_id = ? AND completed = ?', [
    req.user.id,
    1
  ], function (err) {
    if (err) { return next(err); }
